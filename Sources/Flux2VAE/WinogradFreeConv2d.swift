...
// (conv_in 32→512, the 512/256/128-ch resnets, the upsamplers). Probe: `swift test --filter
// WinogradProbeTests` (weight-free). Removal: when the probe reports raw conv2d exact on a new
// mlx-swift pin, go back to plain Conv2d. `FLUX2VAE_WINOGRAD=1` restores the raw conv2d path
// (validation only).
...
final class WinogradFreeConv2d: Conv2d {
    ...
    var enabled = getenv("FLUX2VAE_WINOGRAD") == nil
