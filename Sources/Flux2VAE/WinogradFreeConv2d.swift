...
//
// The FLUX.2 decoder hits the window in nearly every 3×3 conv: at 1024² that is 32 convs
// (conv_in 32→512, the 512/256/128-ch resnets, the upsamplers). Default `.winograd`: the fp32
// loss is below 8-bit visibility and `.conv3d` costs +750 ms per 1024² decode; parity lanes opt
// in, and bf16 decodes (ERNIE) can take `.fp32Winograd`.
...
// `FLUX2VAE_CONV_ROUTE=winograd|conv3d|fp32Winograd` overrides the defaults (validation).
...
public enum Flux2VAEConvRoute: String, Sendable {
...
final class WinogradFreeConv2d: Conv2d {
    var route: Flux2VAEConvRoute = .conv3d
